...
const controllerSource = fs.readFileSync(
  path.join(repoRoot, 'src', 'api', 'integrations', 'event', 'event.controller.ts'),
  'utf8',
);
...
const wireEvents = new Map(
  [...enumSource.matchAll(/^\s+([A-Z0-9_]+) = '([^']+)'/gm)].map((match) => [match[1], match[2]]),
);
...
if (missing.length || stale.length) {
  throw new Error(
    `Webhook documentation event mismatch. Missing: ${missing.join(', ') || 'none'}. Stale: ${stale.join(', ') || 'none'}.`,
  );
}
...
for (const eventName of configuredEvents) {
  ...
  if (!html.includes(wireName))
    throw new Error(`Generated HTML does not contain ${wireName}; regenerate docs/webhooks.`);
}
